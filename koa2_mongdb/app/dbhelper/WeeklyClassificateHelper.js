...
exports.find = async ({_id}) => {
  var res = []
  await WeeklyClassificate.find({"_id":_id},function(err, WeeklyClassificate) {
		if(err) {
			res = []
		}else {
			res = WeeklyClassificate;
...
exports.update = async (wherestr, updatestr) => {
	var res = []
	await WeeklyClassificate.update(wherestr, updatestr,function(err, WeeklyClassificate){
		if(err) {
			res = []
		}else {
			return WeeklyClassificate
